...
class WsAccessory extends homebridgeLib.AccessoryDelegate {
  constructor (platform, context) {
    ...
    this.historyService = new homebridgeLib.ServiceDelegate.History.Sensor(
      this, {
        motionDelegate: this.wsServices.rain.characteristicDelegate('motion'),
        lastMotionDelegate: this.wsServices.rain.characteristicDelegate('lastActivation'),
  ...
  async heartbeat (beat) {
    ...
    if (beat % heartrate === 1) {
      try {
        ...
        this.wsServices.rain.checkWeather(this.wsServices.weather)
      } catch (error) {
