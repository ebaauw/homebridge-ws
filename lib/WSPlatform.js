...
function WSPlatform (log, configJson, api) {
  ...
  this.wunderground = new RestClient({
...
WSPlatform.prototype.accessories = function (callback) {
  ...
  npmRegistry.get(packageJson.name).then((response) => {
    ...
    if (this.config.apiKey === undefined) {
      this.log.error('config.json: error: apikey missing')
      return callback(accessoryList)
    }
    const station = new WSAccessory(this, this.config.name, this.config.location)
