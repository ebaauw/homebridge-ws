...
function WSAccessory (platform, name, location) {
  this.log = platform.log
  this.platform = platform
  this.name = name
  this.location = location + '.json'
  this.displayName = name
  this.serialNumber = 'WS-' + location.toUpperCase().replace(/[^A-Z0-9]/g, '')
  ...
  this.historyService = new HistoryService('weather', {displayName: this.name}, {
    disableTimer: true,
    storage: 'fs',
...
WSAccessory.prototype.heartbeat = function (beat) {
  if (beat % this.state.heartrate === 0) {
    this.platform.wunderground.get(this.location).then((response) => {
      ...
      // this.log.debug('%s: %j', this.name, response)
      ...
      this.log.error('%s: %s', this.name, err)
    })
  }
  if (beat % 600 === 5) {
    this.entry.time = moment().unix()
    this.log.debug('%s: add history entry %j', this.name, this.entry)
