...
class WsPlatform extends Platform {
  ...
  async onecall4 (lat, lon) {
    const response = await this._client.get(
      `4.0/onecall/current?lat=${lat}&lon=${lon}`
    )
    ...
    {
      // We need daily forecasts for weather service.
      const response = await this._client.get(
        `4.0/onecall/timeline/1day?lat=${lat}&lon=${lon}`
      )
      observation.body.daily = this.checkResponse(response).body.data
    }
    if (this.config.hourlyForecasts > 0) {
      const response = await this._client.get(
        `4.0/onecall/timeline/1h?lat=${lat}&lon=${lon}`
      )
      observation.body.hourly = this.checkResponse(response).body.data
    }
    return observation
  }
